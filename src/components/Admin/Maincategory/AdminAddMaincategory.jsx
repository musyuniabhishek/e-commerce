...
export default function AdminAddMaincategory() {
  ...
  let maincategoryElement = useRef();
  let allStateData = useSelector((state) => state.MaincategoryStateData);

  const postData = (e) => {
    e.preventDefault();
    let maincategory = maincategoryElement.current.value;
    if (allStateData.slice(1).find((item) => item.name === maincategory)) {
      alert("Maincategory name already added");
    } else {
      dispatch(createMaincategory({ name: maincategory }));
